...
def detect_and_display(capture):
    frame_gray = cv.cvtColor(capture, cv.COLOR_BGR2GRAY)
    ...
    for (x, y, w, h) in faces:
        center = (x + w // 2, y + h // 2)
        capture = cv.ellipse(
            capture, center, (w // 2, h // 2), 0, 0, 360, (255, 0, 255), 4)
        ...
        for (x2, y2, w2, h2) in eyes:
            ...
            capture = cv.circle(capture, eye_center, radius, (255, 0, 0), 4)

    cv.imshow('Capture - Face detection', capture)
...
while True:
    ...
    if cv.waitKey(10) == 27:
        break
cv.destroyWindow('Capture - Face detection')
cap.release()
