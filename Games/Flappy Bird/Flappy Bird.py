...
def main():
    ...
    pipe_images['bottom'] = pygame.image.load(random.choice(
        list(cfg.PIPE_IMAGE_PATHS.values()))).convert_alpha()
    ...
    background_image = pygame.image.load(random.choice(
        list(cfg.BACKGROUND_IMAGE_PATHS.values()))).convert_alpha()
    ...
    game_start_info = startGame(
        screen, sounds, bird_images, other_images, background_image, cfg)
    ...
    for i in range(2):
        ...
        pipe_sprites.add(Pipe(image=pipe_images.get('top'), position=(
            cfg.SCREENWIDTH+200+i*cfg.SCREENWIDTH/2, pipe_pos.get('top')[-1])))
        pipe_sprites.add(Pipe(image=pipe_images.get('bottom'), position=(
    ...
    while is_game_running:
        ...
        is_dead = bird.update(
            boundary_values, float(clock.tick(cfg.FPS))/1000.)
        if is_dead:
            sounds['hit'].play()
            is_game_running = False
        ...
        for pipe in pipe_sprites:
            ...
            if pipe.rect.left < 5 and pipe.rect.left > 0 and is_add_pipe:
                pipe_pos = Pipe.randomPipe(cfg, pipe_images.get('top'))
                pipe_sprites.add(Pipe(image=pipe_images.get(
                    'top'), position=pipe_pos.get('top')))
                pipe_sprites.add(Pipe(image=pipe_images.get(
                    'bottom'), position=pipe_pos.get('bottom')))
                is_add_pipe = False
    ...
    endGame(screen, sounds, showScore, score, number_images, bird,
            pipe_sprites, background_image, other_images, base_pos, cfg)
