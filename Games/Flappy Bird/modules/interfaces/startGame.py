...
def startGame(screen, sounds, bird_images, other_images, background_image, cfg):
    ...
    msg_pos = [(cfg.SCREENWIDTH-other_images['message'].get_width()
                )/2, cfg.SCREENHEIGHT*0.12]
    bird_idx = 0
    bird_idx_change_count = 0
    bird_idx_cycle = itertools.cycle([0, 1, 2, 1])
    bird_pos = [cfg.SCREENWIDTH*0.2,
                (cfg.SCREENHEIGHT-list(bird_images.values())[0].get_height())/2]
